...
export default function ToursTab() {
  ...
  const autoGenerateCompleteMutation = trpc.tours.autoGenerateComplete.useMutation({
    ...
    onSuccess: (result) => {
      setExtractionStep(4); // 生成完成
      setIsGenerating(false);
      setCurrentTaskId(null);
      setIsAutoGenerateDialogOpen(false); // 關閉生成對話框
      setAutoGenerateUrl(""); // 清空 URL
      setExtractionStep(0); // 重置步驟
      ...
      if (result.success) {
        toast.success(
          result.message || "行程已成功生成並儲存！",
          {
            description: result.tourId ? `行程 ID: ${result.tourId}` : undefined,
            duration: 5000,
          }
        );
      }
  ...
  const handleAutoGenerate = () => {
    ...
    // 使用自動儲存模式，生成完成後直接儲存到資料庫
    autoGenerateCompleteMutation.mutate({ 
      url: autoGenerateUrl,
      previewOnly: false, // 關閉預覽模式
      autoSave: true, // 啟用自動儲存
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isAutoGenerateDialogOpen} onOpenChange={(open) => {
        if (!open && !autoGenerateCompleteMutation.isPending && !isGenerating) {
        ...
        <DialogContent className="max-w-lg rounded-3xl">
          ...
          <DialogFooter>
            <Button
              ...
              disabled={autoGenerateCompleteMutation.isPending || isGenerating}
              className="rounded-full"
            >
              取消
            </Button>
            <Button
              ...
              disabled={autoGenerateCompleteMutation.isPending || isGenerating || !autoGenerateUrl.trim()}
              className="bg-purple-600 text-white hover:bg-purple-700 rounded-full"
            >
              {(autoGenerateCompleteMutation.isPending || isGenerating) ? (
                <>
